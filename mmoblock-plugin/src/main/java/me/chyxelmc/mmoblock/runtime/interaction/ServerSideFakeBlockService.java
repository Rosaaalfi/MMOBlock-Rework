...
public final class ServerSideFakeBlockService {
    ...
    public void demoteBlock(final PlacedBlockModel block) {
        ...
        demote(world, new PositionKey(block.world(), (int) Math.floor(block.originX()), (int) Math.floor(block.originY()), (int) Math.floor(block.originZ())), true);
        demote(world, new PositionKey(block.world(), (int) Math.floor(block.x()), (int) Math.floor(block.y()), (int) Math.floor(block.z())), true);
    }
    ...
    private Set<PositionKey> collectNearFakeBlocks(final World world, final Location playerLocation, final double radiusSquared) {
        ...
        for (final String rawKey : FakeBlockRegistry.positionsForWorld(world.getName())) {
            ...
            // Skip positions that are registered as additional (schematic/collision) positions.
            // These are multi-block parts managed via NMS fake-block packets, NOT through the
            // server-side fake-block promote/demote system. Promoting them would change the
            // actual world block data, leaving permanent debris when the schematic is cleared.
            if (this.stateRegistry.isAdditionalPosition(position.worldName(), position.x(), position.y(), position.z())) {
                continue;
            }
    ...
    private void demote(final World world, final PositionKey position, final boolean scheduleDelayedRefresh) {
        final PromotedBlock promoted = this.promotedBlocks.remove(position);
        this.promotedPositions.remove(position);
        if (promoted == null) {
            return;
        }
