...
public final class GuiInventoryAccessImpl implements GuiInventoryAccess {
    @Override public boolean openCustomInventory(final Player player, final Inventory inventory, final String title) { final ServerPlayer handle=((CraftPlayer)player).getHandle(); if(handle.connection==null)return false; final MenuType<?> type=CraftContainer.getNotchInventoryType(inventory); AbstractContainerMenu menu=new CraftContainer(inventory,handle,handle.nextContainerCounter()); menu=CraftEventFactory.callInventoryOpenEvent(handle,menu); if(menu==null)return false; menu.checkReachable=false; handle.connection.send(new ClientboundOpenScreenPacket(menu.containerId,type,Component.literal(title))); handle.containerMenu=menu; handle.initMenu(menu); return true; }
    @Override public boolean updateTitle(final Player player, final String title) { final ServerPlayer handle=((CraftPlayer)player).getHandle(); final AbstractContainerMenu menu=handle.containerMenu; if(handle.connection==null||menu==null)return false; handle.connection.send(new ClientboundBundlePacket(List.of(new ClientboundOpenScreenPacket(menu.containerId,menu.getType(),Component.literal(title)),new ClientboundContainerSetContentPacket(menu.containerId,menu.incrementStateId(),menu.getItems(),menu.getCarried())))); return true; }
