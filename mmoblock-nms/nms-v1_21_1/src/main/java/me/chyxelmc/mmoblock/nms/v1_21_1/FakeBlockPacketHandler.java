    ...
    private int[] tryExtractChunkCoords(final Object packet) {
        ...
        try {
            try {
                ...
                // accessor methods not found; fall through to field scanning
            }
            ...
            for (final Field f : packet.getClass().getDeclaredFields()) {
                try {
                    ...
                    if ((name.contains("chunkx") || name.equals("x") || name.equals("chunkx")) && val instanceof Integer ix) {
                        cx = ix;
                    }
                    if ((name.contains("chunkz") || name.equals("z") || name.equals("chunkz")) && val instanceof Integer iz) {
                        cz = iz;
                    }
                    if (cx != null && cz != null) return new int[]{cx, cz};
                } catch (final Exception ignored) {
                    // field not accessible; continue
                }
            }
        } catch (final Exception ignored) {
            // packet class not accessible; skip
        }
        return null;
    }
    ...
    private net.minecraft.world.level.block.state.BlockState computeSendState(final ServerLevel level, final BlockPos pos, final String materialName) {
        if (materialName != null) {
            try {
                ...
                // invalid material name; fall through to level state
            }
        }
        try {
    ...
    private void handleInterceptedPacket(final Object msg) {
        ...
        try {
            if (isFakeBlockOrContains(player, pos)) {
                sendFakeRefreshToPlayer(player, ((CraftWorld) player.getWorld()).getHandle(), pos);
