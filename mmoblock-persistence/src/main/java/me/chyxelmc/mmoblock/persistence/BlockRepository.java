...
public final class BlockRepository {
    ...
    public void upsert(final PlacedBlockModel block) {
    ...
    public List<PlacedBlockModel> findAll() {
        if (!this.dataCache.isAllBlocksStale(ALL_BLOCKS_TTL_MS)) {
            final List<PlacedBlockModel> cached = this.dataCache.getAllBlocks();
            if (cached != null) {
                return cached;
            }
        }
        final List<PlacedBlockModel> blocks = new ArrayList<>();
        ...
        try (Connection connection = this.databaseManager.getConnection();
            ...
            while (resultSet.next()) {
                ...
                blocks.add(new PlacedBlockModel(
